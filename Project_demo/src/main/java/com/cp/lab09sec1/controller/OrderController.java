...
public class OrderController {
    ...
    @GetMapping("/byDate")
    public ResponseEntity<Object> getOrdersByDate(@org.springframework.web.bind.annotation.RequestParam String date) {
        String targetUrl = DATA_SERVICE_URL + "/api/orders/byDate?date=" + date;

        try {
            // forward ไปยัง Data Service (8085)
            ResponseEntity<Object> response = restTemplate.getForEntity(targetUrl, Object.class);
            return ResponseEntity.status(response.getStatusCode()).body(response.getBody());
        } catch (Exception e) {
            // ถ้า Data Service ล่ม หรือไม่ตอบสนอง
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("โหลดข้อมูลยอดขายไม่สำเร็จ: " + e.getMessage());
        }
    }
